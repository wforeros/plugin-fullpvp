...
public class KitSetMaximumUsesArgument extends CommandArgument {
    private final FullPvP plugin;

    public KitSetMaximumUsesArgument(FullPvP plugin) {
        super("setmaximumuses", "Sets the maximum uses for a kit.", new String[]{"maxuses", "maximumuses", "setsetmaxuses"});
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (args.length < 3) {
            ...
            if (kit == null) {
                ...
                sender.sendMessage(ChatColor.YELLOW + "The maximum usages of kit " + ChatColor.GREEN + kit.getName() + ChatColor.YELLOW + " have been set to " + ChatColor.AQUA + (amount == 2147483647 ? "unlimited" : amount) + ChatColor.YELLOW + '.');
                return true;
            }
        }
    }
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        if (args.length != 2) {
            return args.length == 3 ? Arrays.asList("unlimited") : Collections.emptyList();
        } else {
