...
public class KitSetIconArgument extends CommandArgument {
    private final FullPvP plugin;

    public KitSetIconArgument(FullPvP plugin) {
        super("seticon", "Change the display icon of a kit.", new String[]{"icon"});
        this.plugin = plugin;
            ...
            if (args.length < 2) {
                ...
                if (kit == null) {
                    ...
                    if (item != null && item.getType() != Material.AIR) {
                        kit.setIcon(item);
                        player.sendMessage(ChatColor.YELLOW + "The kit " + ChatColor.GREEN + kit.getName() + ChatColor.YELLOW + " will now be displayed as a " + ChatColor.AQUA + this.plugin.getNightmare().getItemDb().getName(item) + ChatColor.YELLOW + '.');
                    } else {
