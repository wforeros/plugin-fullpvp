...
public class ClaimToggleFlyArgument extends CommandArgument {
    private FullPvP plugin;

    public ClaimToggleFlyArgument(FullPvP plugin) {
        super("fly", "Modify fly permission in land.", new String[]{""});
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] arguments) {
        if (arguments.length != 3) {
            ...
            if (arguments[2].equalsIgnoreCase("enable")) {
                (this.plugin.getClaimHandler().getClaimsList().get(arguments[1])).setFlyAllowed(true);
                sender.sendMessage(ChatColor.BLUE + "You have " + ChatColor.GREEN + "enabled" + ChatColor.BLUE + " fly command into this claim.");
            } else if (arguments[2].equalsIgnoreCase("disable")) {
                (this.plugin.getClaimHandler().getClaimsList().get(arguments[1])).setFlyAllowed(false);
                sender.sendMessage(ChatColor.BLUE + "You have " + ChatColor.RED + "disabled" + ChatColor.BLUE + " fly command into this claim.");
            } else {
