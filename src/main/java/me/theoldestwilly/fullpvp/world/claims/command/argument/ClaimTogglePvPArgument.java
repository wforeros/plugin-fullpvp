...
public class ClaimTogglePvPArgument extends CommandArgument {
    private FullPvP plugin;

    public ClaimTogglePvPArgument(FullPvP plugin) {
        super("pvp", "Modify pvp permission in land.", new String[]{""});
        this.plugin = plugin;
    }
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] arguments) {
        if (arguments.length != 3) {
            ...
            if (arguments[2].equalsIgnoreCase("enable")) {
                (this.plugin.getClaimHandler().getClaimsList().get(arguments[1])).setPvpAllowed(true);
                sender.sendMessage(ChatColor.BLUE + "You have " + ChatColor.GREEN + "enabled" + ChatColor.YELLOW + "" + ChatColor.BOLD + " PvP " + ChatColor.BLUE + "into this claim.");
            } else if (arguments[2].equalsIgnoreCase("disable")) {
                (this.plugin.getClaimHandler().getClaimsList().get(arguments[1])).setPvpAllowed(false);
                sender.sendMessage(ChatColor.BLUE + "You have " + ChatColor.RED + "disabled" + ChatColor.YELLOW + "" + ChatColor.BOLD + " PvP " + ChatColor.BLUE + "into this claim.");
            } else {
